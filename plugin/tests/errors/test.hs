...
spec = describe "the fixtures" do
  ...
  -- The old spelling separated the value with a colon. It is now an option name
  -- that does not exist, and says so, rather than being quietly ignored.
  it "rejects the old colon spelling as an unknown option" $
    (outDir, "Ok.hs", ["-fplugin-opt", "NoRecursion:allowRecursion:true"])
      `shouldResultIn` Invalid
        ["unknown plugin option", "allowRecursion:true"]

  -- The fixture’s own pragma says @allowRecursion=true@ and comes after this
  -- flag, so the pragma has to win.
